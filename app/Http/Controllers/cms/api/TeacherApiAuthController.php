...
class TeacherApiAuthController extends Controller
{
    //
    ...
    public function login(Request $request){
        ...
        if (!$validator->fails()) {
            $teacher = Teacher::where('id', $request->get('id'))->first();
            $this->revokeActiveTokens($teacher->id);
            $response = Http::asForm()
                ->post('http://127.0.0.1:8081/oauth/token', [
                    'grant_type' => 'password',
                    'client_id' => '4',
                    'client_secret' => 'UFi001c2EGAEJXUtr1NG0uNhID9tyJ4Km9Kw63jE',
                    'username' => $request->get('id'),
                    'password' => $request->get('password'),
                    'scope' => '*',
                ]);
            $teacher->setAttribute('token', $response->json()['access_token']);
            $teacher->setAttribute('refresh_token', $response->json()['refresh_token']);
            return response()->json(['status' => true, 'message'=>'تم تسجيل الدخول بنجاح', 'teacher' => $teacher], 200);
        }else{
    ...
    public function logout(Request $request){
        $token = $request->user('teacherApi')->token();
